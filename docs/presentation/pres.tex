...
\begin{frame}
    ...
    \begin{block}{PYNQ}
    \onslide<2->PYNQ is an open-source project which provides a Python-based development environment for Xilinx Zynq SoCs.
    \end{block}
...
\begin{frame}
    ...
    \begin{block}{AES}
    \onslide<2->AES is a symmetric-key algorithm for the encryption of electronic data. It is widely used in both industry and government to protect sensitive data.
    \end{block}
...
\begin{frame}
    ...
    \begin{itemize}
        \onslide<2->\item Works on every platform supported by PYNQ (with the correct bitstream)
