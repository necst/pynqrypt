# This file is automatically generated.
# It contains project source information necessary for synthesis and implementation.

# Block Designs: bd/main_design/main_design.bd
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design || ORIG_REF_NAME==main_design} -quiet] -quiet

# IP: bd/main_design/ip/main_design_pynqrypt_encrypt_0_0/main_design_pynqrypt_encrypt_0_0.xci
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design_pynqrypt_encrypt_0_0 || ORIG_REF_NAME==main_design_pynqrypt_encrypt_0_0} -quiet] -quiet

# IP: bd/main_design/ip/main_design_processing_system7_0_0/main_design_processing_system7_0_0.xci
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design_processing_system7_0_0 || ORIG_REF_NAME==main_design_processing_system7_0_0} -quiet] -quiet

# IP: bd/main_design/ip/main_design_ps7_0_axi_periph_0/main_design_ps7_0_axi_periph_0.xci
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design_ps7_0_axi_periph_0 || ORIG_REF_NAME==main_design_ps7_0_axi_periph_0} -quiet] -quiet

# IP: bd/main_design/ip/main_design_rst_ps7_0_100M_0/main_design_rst_ps7_0_100M_0.xci
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design_rst_ps7_0_100M_0 || ORIG_REF_NAME==main_design_rst_ps7_0_100M_0} -quiet] -quiet

# IP: bd/main_design/ip/main_design_xbar_0/main_design_xbar_0.xci
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design_xbar_0 || ORIG_REF_NAME==main_design_xbar_0} -quiet] -quiet

# IP: bd/main_design/ip/main_design_axi_mem_intercon_0/main_design_axi_mem_intercon_0.xci
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design_axi_mem_intercon_0 || ORIG_REF_NAME==main_design_axi_mem_intercon_0} -quiet] -quiet

# IP: bd/main_design/ip/main_design_auto_pc_0/main_design_auto_pc_0.xci
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design_auto_pc_0 || ORIG_REF_NAME==main_design_auto_pc_0} -quiet] -quiet

# IP: bd/main_design/ip/main_design_auto_ds_0/main_design_auto_ds_0.xci
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design_auto_ds_0 || ORIG_REF_NAME==main_design_auto_ds_0} -quiet] -quiet

# IP: bd/main_design/ip/main_design_auto_pc_1/main_design_auto_pc_1.xci
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design_auto_pc_1 || ORIG_REF_NAME==main_design_auto_pc_1} -quiet] -quiet

# IP: bd/main_design/ip/main_design_auto_ds_1/main_design_auto_ds_1.xci
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design_auto_ds_1 || ORIG_REF_NAME==main_design_auto_ds_1} -quiet] -quiet

# IP: bd/main_design/ip/main_design_auto_pc_2/main_design_auto_pc_2.xci
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design_auto_pc_2 || ORIG_REF_NAME==main_design_auto_pc_2} -quiet] -quiet

# IP: bd/main_design/ip/main_design_auto_ds_2/main_design_auto_ds_2.xci
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design_auto_ds_2 || ORIG_REF_NAME==main_design_auto_ds_2} -quiet] -quiet

# IP: bd/main_design/ip/main_design_auto_pc_3/main_design_auto_pc_3.xci
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design_auto_pc_3 || ORIG_REF_NAME==main_design_auto_pc_3} -quiet] -quiet

# IP: bd/main_design/ip/main_design_s00_mmu_0/main_design_s00_mmu_0.xci
set_property KEEP_HIERARCHY SOFT [get_cells -hier -filter {REF_NAME==main_design_s00_mmu_0 || ORIG_REF_NAME==main_design_s00_mmu_0} -quiet] -quiet

# XDC: /home/mrindeciso/Documents/pynqrypt/pynqrypt-vivado/pynqrypt-vivado.gen/sources_1/bd/main_design/main_design_ooc.xdc
