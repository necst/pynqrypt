...
typedef struct {
    ...
    XPynqrypt_encrypt_uio_map maps[ MAX_UIO_MAPS ];
} XPynqrypt_encrypt_uio_info;

/***************** Variable Definitions **************************************/
static XPynqrypt_encrypt_uio_info uio_info;
...
static int uio_info_read_name(XPynqrypt_encrypt_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPynqrypt_encrypt_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPynqrypt_encrypt_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPynqrypt_encrypt_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPynqrypt_encrypt_Initialize(XPynqrypt_encrypt *InstancePtr, const char* InstanceName) {
	XPynqrypt_encrypt_uio_info *InfoPtr = &uio_info;
